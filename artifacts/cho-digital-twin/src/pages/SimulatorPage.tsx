...
const PARAM_GROUPS: { title: string; eq: string; fields: ParamField[] }[] = [
  {
    ...
    fields: [
      ...
      { key: "kl1", label: "k_l¹", units: "day⁻¹·(Mc/mL)⁻¹", min: 0, max: 0.1, step: 0.0001, digits: 6 },
    ],
  },
  {
    title: "Glucose",
  ...
  },
  {
    ...
    eq: "titer",
    fields: [
      { key: "q_p", label: "q_p", units: "mg·L⁻¹·Mc⁻¹·d⁻¹", min: 0, max: 0.1, step: 0.001, digits: 4 },
    ],
  },
];
...
function ParamEditor({
  ...
  const isDefault = (key: keyof ModelParams) =>
  ...
  return (
    <div className="param-editor">
      {PARAM_GROUPS.map((group) => {
        ...
        const anyModified = group.fields.some((f) => !isDefault(f.key));
        return (
          <div key={group.title} className={`param-group ${anyModified ? "param-group-modified" : ""}`}>
            <button className="param-group-header" onClick={() => toggleGroup(group.title)}>
              <span className="param-group-name">
                {group.title}
                {anyModified && <span className="param-modified-dot" title="Modified from Table 1">●</span>}
              </span>
                    ...
                    <div key={f.key} className={`param-row ${modified ? "param-row-modified" : ""}`}>
                      <div className="param-row-top">
                        ...
                        {modified && (
                          <button
                            ...
                            title={`Reset to Table 1 default: ${def.toExponential(4)}`}
                            onClick={() => onChange(f.key, def)}
                          >↺</button>
                        )}
                      </div>
                      ...
                      {modified && (
                        <div className="param-default-hint">
                          Table 1: {def.toExponential ? def.toPrecision(7) : def}
                        </div>
                      )}
                    </div>
                  );
...
export default function SimulatorPage() {
  ...
  // Sigmoid growth rate components
  ...
  // Model parameters (mutable, start from Table 1 defaults)
  ...
  const runSim = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      try {
        setResults(runSimulation(config));
      } catch (err) {
        console.error("Simulation error:", err);
      }
      setRunning(false);
    }, 10);
  }, [Xv0, Glc0, Lac0, Gln0, Glu0, NH40, feedEnabled, sigA1, sigC1, sigA2, sigC2, modelParams]);
    ...
    <div className="sim-page">
      ...
      <aside className="sim-controls">
        ...
        <section className="ctrl-section">
          ...
          <p className="ctrl-hint">
            μ_net(t) = Σ aₖ · σ(bₖ · (t − cₖ)).  Full NN (§2.3) needs 23-batch dataset.
          </p>
        ...
        <section className="ctrl-section">
          <div className="params-header">
            <h3 className="ctrl-sect-title" style={{ margin: 0 }}>
              ...
              {anyParamModified && <span className="param-modified-dot" title="Some parameters differ from Table 1">●</span>}
            </h3>
            {anyParamModified && (
              <button className="reset-all-btn" onClick={resetAllParams}>
                Reset all to Table 1
              </button>
            )}
          </div>
          <p className="ctrl-hint">
            Click a group to expand and edit. Modified values shown with an indicator.
            Type or drag to change — slider and number input stay in sync.
          </p>
      ...
      <main className="sim-results">
        ...
        <div className="charts-grid">
          ...
          <ChartPanel title="Product Titer (mAb)" data={results}
